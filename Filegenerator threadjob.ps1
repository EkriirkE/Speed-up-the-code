...
$mx=50000
$c=[System.Math]::Floor($mx/[int]$env:NUMBER_OF_PROCESSORS)
$th=[System.Collections.ArrayList]@()
While($mx -gt 0){
	$th.Add((Start-ThreadJob -ScriptBlock $PS -ArgumentList ([System.Math]::Max(0,$mx-$c)),($mx-1),$work))
	$mx-=$c
}#Might get a remainder thrown in a CPU+1 thread.  Naja

$th | Receive-Job -Wait -AutoRemoveJob
