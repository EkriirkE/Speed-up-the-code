...
ForEach ($_ in 0..49999){	#Faster than for(;;) or (x..y).ForEach.  Do I dare to unroll the loop :o
	$seed=$random.Next()	#Get a random number for further pseudo-pseudo random extractions.  Output is still random.
	$timestamp=$date.AddSeconds(-$i).ToString("yyyy-MM-dd HH:mm:ss")	#IDK what to speed up here
	$plc=$plcNames[$seed%4]
	$operator=$seed%20+101
	$batch=$seed%101+1000
	$status=$statusCodes[$seed%3]
	$seed=$random.Next()	#Need to refresh because we already extract %101
	...
	if($seed%7){	#6:7 chance of normality
		...
		$output.AppendLine("ERROR; {0}; {1}; {2}; {3}; {4}; {5}; {6}; {7}; {8}" -f ($timestamp,$plc,$errorTypes[$errorType],($seed%10+1),$status,$operator,$batch,$machineTemp,$load))
	}
}
